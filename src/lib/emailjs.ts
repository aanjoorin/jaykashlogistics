...
export const sendCustomerConfirmationEmail = async (templateId: string, data: any) => {
  try {
    ...
    // Add thank you message (customer-specific part)
    ...
    messageContent += `Here's a complete summary of the information you provided:\n\n`;
    ...
    // Closing message (customer-specific part)
    ...
    // IMPORTANT: These parameter names must match EXACTLY what your EmailJS template expects
    const emailData = {
      // The recipient email - this is the key parameter for the "To Email" field
      to_email: data.shipper_info.email,
      
      // Common template variables - adjust these to match your template
      customer_name: data.shipper_info.name,
      service_type: data.service_type,
      message: messageContent,
      subject: `Your Quote Request - ${data.service_type}`,
      
      // Additional fields that might be used in your template
      shipper_name: data.shipper_info.name,
      shipper_email: data.shipper_info.email,
      shipper_phone: data.shipper_info.phone,
      
      // Vehicle details if available
      vehicle_details: data.vehicle_info ? 
        `${data.vehicle_info.year} ${data.vehicle_info.make} ${data.vehicle_info.model}` : 
        'Not provided',
        
      // Add any other fields your template might be using
      email: data.shipper_info.email, // Some templates might use 'email' instead of 'to_email'
    };
    
    console.log('Sending customer confirmation email to:', data.shipper_info.email);
